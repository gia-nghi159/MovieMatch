...
const ResultPage = () => {
  ...
  const getReasonText = () => {
    if (!winner) return '';
    return `${winner.title} received the strongest support from the group with ${
      winner.likes ?? 0
    } like(s) and ${winner.dislikes ?? 0} dislike(s).`;
  };
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-7 items-stretch">
              <div className="bg-white/10 rounded-[22px] p-5 flex flex-col gap-4">
                {winner.posterPath ? (
                  <img
                    ...
                    className="w-full min-h-[360px] lg:min-h-[500px] object-cover rounded-[18px]"
                  />
                ) : (
              ...
              <div className="bg-white/10 rounded-[22px] p-6 md:p-7">
                ...
                <div className="flex flex-wrap gap-2.5 mb-6">
                  ...
                  <span className="bg-[#ffd369]/12 border border-[#ffd369]/35 text-white px-4 py-2 rounded-full text-[14px]">
                    Dislikes: {winner.dislikes ?? 0}
                  </span>
                </div>

                <div className="mb-6">
                  <h3 className="text-[21px] font-bold text-[#ffd369] mb-2.5">
                    Why This Movie Was Selected
                  </h3>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                    <div className="bg-white/10 rounded-[16px] p-4">
                      <strong className="block mb-2 text-[#ffd369] text-[15px]">
                        TMDB ID
                      </strong>
                ...
                {runnerUps.length > 0 && (
                  <div className="mb-2">
                    <h3 className="text-[21px] font-bold text-[#ffd369] mb-3">
                      Runner-Ups
                    </h3>
                    ...
                    <div className="flex flex-col gap-3">
                      {runnerUps.map((movie, index) => (
                        <div
                          key={movie.ID || index}
                          className="bg-white/10 rounded-[14px] px-4 py-3 border border-white/10"
                        >
                          <div className="font-bold text-white text-[16px]">
                            {index + 2}. {movie.title}
                          </div>
                          <div className="text-[14px] text-[#ddd] mt-1">
                            Likes: {movie.likes ?? 0} • Dislikes: {movie.dislikes ?? 0} • TMDB Rating: {movie.voteAverage ?? 'N/A'}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="flex justify-center gap-4 flex-wrap mt-8">
              <button
                ...
                className="min-w-[200px] border-none cursor-pointer px-6 py-[14px] rounded-[12px] text-[16px] font-bold transition-all bg-[#ffd369] text-[#1b1b1b] hover:bg-[#ffbf00]"
              >
                Start New Session
              </button>
              ...
              <button
                onClick={() => navigate('/')}
                className="min-w-[200px] cursor-pointer px-6 py-[14px] rounded-[12px] text-[16px] font-bold transition-all bg-transparent border-2 border-[#ffd369] text-[#ffd369] hover:bg-[#ffd369] hover:text-[#1b1b1b]"
              >
                Return Home
              </button>
            </div>
