...
const SwipeVoting = () => {
  ...
  useEffect(() => {
    ...
    const fetchMovies = async () => {
      try {
        ...
        if (attempts < 5) {
  setLoadingMessage('Finding movies...');
  timeoutId = setTimeout(fetchMovies, 1000);
} else {
  setLoadingMessage('No movies found. Please check again!');
}
      } catch (error) {
        console.error('Failed to load movies:', error);
        setLoadingMessage('Failed to load movies.');
      }
    };
  ...
  if (movies.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#141e30] to-[#243b55] text-white flex justify-center items-center">
        <div className="text-center text-[24px] font-bold text-[#ffd369]">
          {loadingMessage}
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#141e30] to-[#243b55] text-white font-sans flex justify-center items-center p-5">
      <div className="w-full max-w-[1100px] bg-white/10 backdrop-blur-md rounded-[24px] p-[35px] shadow-2xl border border-white/10">
        <div className="flex justify-between items-center gap-5 mb-6 flex-wrap">
          <div>
            <h1 className="text-[34px] font-bold text-[#ffd369] mb-2">
              Swipe Voting
            </h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-[360px_1fr] gap-[30px] items-stretch">
            ...
            <div className="bg-white/10 rounded-[22px] p-7 flex flex-col">
              <h2 className="text-[32px] font-bold text-[#ffd369] mb-3">
                {currentMovie.title}
              </h2>
              ...
              <div className="mb-auto">
                <h3 className="text-[19px] font-bold text-[#ffd369] mb-2">
                  Overview
                </h3>
              ...
              <div className="flex justify-center gap-3 flex-wrap mt-7">
                <button
                  onClick={() => handleVote('dislike')}
                  className="bg-[#ff6b6b] text-white font-bold py-[14px] px-5 rounded-[12px] text-[16px] min-w-[140px] hover:bg-[#ff5252] transition-all transform hover:-translate-y-0.5 shadow-lg"
                >
                  👎 Dislike
                </button>
                ...
                <button
                  ...
                  className="bg-[#2ecc71] text-white font-bold py-[14px] px-5 rounded-[12px] text-[16px] min-w-[140px] hover:bg-[#27ae60] transition-all transform hover:-translate-y-0.5 shadow-lg"
                >
                  👍 Like
                </button>
              </div>
