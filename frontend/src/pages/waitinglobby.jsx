...
const WaitingLobby = ({ roomCode, qrImage, onBack }) => {
  
  const participants = [
    { name: "Host - Alex", status: "Connected" },
    { name: "Jamie", status: "Connected" },
    { name: "Taylor", status: "Connected" },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[25px] mt-[30px]">
          ...
          <div className="bg-white/10 p-[25px] rounded-[18px] shadow-lg flex flex-col">
            ...
            <ul className="flex flex-col gap-3 list-none">
              {participants.map((person, index) => (
                <li key={index} className="flex justify-between items-center bg-white/10 p-[14px] rounded-[12px] hover:bg-white/15 transition-colors">
                  ...
                  <span className="text-[14px] px-3 py-1 bg-[#2ecc71] rounded-full font-medium">
                    {person.status}
                  </span>
                </li>
              ))}
            ...
            <p className="mt-auto pt-5 text-[15px] text-[#ddd]">
              {participants.length} participants connected. Start the session when enough people have joined.
            </p>
          </div>
        </div>
